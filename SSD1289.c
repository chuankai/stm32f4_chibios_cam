...
{
{ 0x00, 0x07, 0x07, 0x1F, 0x1F, 0x7C, 0x78, 0x78, 0x78, 0x78, 0x7C, 0x1F, 0x1F,
		0x07, 0x07, 0x00 },
{ 0x00, 0xE0, 0xE0, 0xF8, 0xF8, 0x3E, 0x1E, 0x1E, 0x1E, 0x1E, 0x3E, 0xF8, 0xF8,
		0xE0, 0xE0, 0x00 }, };
...
{
	rccEnableAHB3(RCC_AHB3Periph_FSMC, DISABLE);
	...
	palSetGroupMode(GPIOD, 0b1100111110110011, 0,
			PAL_MODE_ALTERNATE(12) | PAL_STM32_OSPEED_HIGHEST);
	//FEDCBA9876543210

	palSetGroupMode(GPIOE, 0b1111111110000000, 0,
			PAL_MODE_ALTERNATE(12) | PAL_STM32_OSPEED_HIGHEST);
	//FEDCBA9876543210
}
...
void LCD_Init_SSD1289(void)
{
	...
	LCD_Delay(5);   // 0x2B3F RGB mode	BGR 233f
	...
	LCD_Delay(5);   // Gate Scan Position 0-319
...
static __inline uint16_t LCD_ReadReg(uint8_t LCD_Reg)
{
	// Vlo hodnotu LCD_Reg do LCD_REG
	LCD_REG = LCD_Reg;
	// Pre鋱ta hodnotu registra LCD
	return (LCD_RAM );
}
 ...
 * @retval	None
 */
void LCD_PutChar(uint16_t PosX, uint16_t PosY, unsigned char c,
...
/**
 * @brief 	Vykresli kruh na dan�miesto na LCD
 * @param	None
 * @retval	PosX, PosY: X a suradnica za鋱atku kruhu
 */
void LCD_DrawCalibPoint(uint16_t PosX, uint16_t PosY)
{
	...
	/*
	 // Vykreslenie kruhu (Circle1) pre kalibraciu touch kontrol廨u
	 for(k=0;k<2;k++)
	 {
	 for(l=0;l<4;l++)
	 {
	 ...
	 */
	// Vykreslenie kruhu (Circle2) pre kalibraciu touch kontrol廨u
	for (i = 0; i < 2; i++)
	{
		for (l = 0; l < 16; l++)
		{
...
static void LCD_Delay(uint16_t nCount)
{
	...
	while (nCount--)
	{
		for (TimingDelay = 0; TimingDelay < 60000; TimingDelay++)
			;	 //24000
	}
}
